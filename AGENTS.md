...
- **Optional LMO sidecar:** LMA works alone. When [Local Model Orchestrator](https://github.com/Gargoyle-Apps/local-model-orchestrator) is cloned locally, share **absolute paths** (env or gitignored `integrations/lmo/paths.yaml`). LMO may own hardware/software YAML; LMA always owns the model DB. Resolve with `./scripts/py scripts/lma_paths.py`. Mock/dry-run profiles are rejected unless the operator explicitly passes `--allow-mock`, sets `LMA_ALLOW_MOCK=1`, or creates gitignored `integrations/lmo/allow-mock` for that clone. Keep mock YAML untracked and label all derived results simulated. Opted-in mock hardware never pulls or creates Ollama weights: scans record `is_active=1` as a simulated install and do not deploy IDE config to `$HOME`. Contract: [integrations/lmo/lma-lmo-contract.md](integrations/lmo/lma-lmo-contract.md). Skill: `lma-lmo-sidecar`.
...
For an explicitly requested simulation, rerun with `--allow-mock` or rely on this clone's `integrations/lmo/allow-mock` flag; never describe its budget as the current host. Confirm `simulate_installs` in the JSON before treating DB `is_active` as a real Ollama install.
